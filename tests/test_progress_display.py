...
def _stall_threshold() -> int:
    line = next(
        ln for ln in (AGENT / "lib" / "roomflow.sh").read_text(encoding="utf-8").splitlines()
...
def render(tmp_path: Path, machines: list) -> str:
    """מריץ את `room_draw_writing` **האמיתי** מהקובץ, לא עותק שלו."""
    run_dir = tmp_path / "run"
    run_dir.mkdir()
    (run_dir / "room.json").write_text(json.dumps({"machines": machines}),
                                       encoding="utf-8")
