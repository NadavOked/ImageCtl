...
test('polling refreshes status, marks failed reads, and stops on logout',async()=>{
  ...
  assert.equal(intervals.size,1); await run('loadOverview()'); assert.equal(run('overviewError'),'');
  fixtures['/overview']=new Error('offline'); await run('loadOverview()');
...
test('existing start and stop retain endpoint payloads',async()=>{
  const {run,node,requests}=setup();run('loadOverview=async()=>{}');
...
test('all static includes stay v4.0 and color literals stay in tokens',()=>{
  const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
  for(const s of html.matchAll(/(?:src|href)="[^"?]+\?v=([^"&]+)/g))assert.equal(s[1],'4.0');
  for(const file of ['console.css','console.js','index.html']) {
    ...
    assert.doesNotMatch(s,/[?]v=(?!4\.0\b)\d/,file);
  }
});
