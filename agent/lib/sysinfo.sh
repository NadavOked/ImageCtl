...
build_hello() {
    ...
    printf '{"schema":1,"mac":"%s","all_macs":[%s],"ip":"%s","hostname_current":null,"uuid":%s,"firmware":"%s","secure_boot":%s,"agent_version":"%s","memory_bytes":%s,"joining":%s,"disks":[%s],"disk_probe":"%s"}' \
