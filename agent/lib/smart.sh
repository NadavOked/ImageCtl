...
#: מעל כמה שגיאות CRC (מאפיין 199) מתייגים `warn`. לרוב זו בעיית כבל/בק
#: ולא הפלטטר עצמו, ולכן סף ולא "כל ערך" — מתחתיו מתעלמים (הכרעת נדב, #652).
SMART_CRC_WARN="${SMART_CRC_WARN:-50}"
...
smart_classify() {
    ...
    # הקורא מנרמל את המספרים לשלמים (ריק -> 0) לפני שהוא קורא לה, כדי
    # שהיא תהיה נבדקת שורה-שורה בלי smartctl.
    if [ "$1" = "FAILED" ]; then echo "fail health_failed"; return; fi
    [ "$4" -gt 0 ] && { echo "fail uncorrectable"; return; }
    [ "$3" -gt 0 ] && { echo "fail pending"; return; }
    [ "$2" -gt 0 ] && { echo "warn reallocated"; return; }
    [ "$5" -ge "$SMART_CRC_WARN" ] && { echo "warn crc"; return; }
    if [ "$1" = "PASSED" ]; then echo "ok ok"; return; fi
    # ‏-H לא PASSED ולא FAILED, וכל המאפיינים תקינים: לא הצלחנו לקרוא
    # ‏verdict בריאות. זה **אינו** `ok` (עיקרון 5) — `unchecked`.
    echo "unchecked no_health"
}

_smart_int() {
    # מדפיס את ערך ה-RAW של מאפיין SMART לפי שם, כמספר שלם, או 0.
    # ‏$1 = קובץ פלט של `smartctl -A`, $2.. = שמות מאפיין אפשריים.
    # ה-RAW הוא העמודה האחרונה; ממנו נגזרים רק הספרות המובילות
    # (‏"35 (Min/Max ..)" של טמפרטורה, "0" של השאר).
    _si_f="$1"; shift
    for _si_name in "$@"; do
        _si_v=$(awk -v n="$_si_name" \
            '$2 == n { v=$NF; sub(/[^0-9].*/, "", v); if (v != "") print v; exit }' \
            "$_si_f" 2>/dev/null)
...
smart_probe() {
    ...
    case "$(grep -iE 'overall-health|SMART Health Status' "$_sp_out" 2>/dev/null)" in
        *FAILED*|*BAD*) _sp_h=FAILED ;;
        *PASSED*|*OK*)  _sp_h=PASSED ;;
        *)              _sp_h=unknown ;;
    esac
    _sp_re=$(_smart_int "$_sp_out" Reallocated_Sector_Ct Reallocated_Event_Count)
    _sp_pe=$(_smart_int "$_sp_out" Current_Pending_Sector)
    # NVMe אינו נושא 197/198; "Media and Data Integrity Errors" הוא המקבילה.
    _sp_un=$(_smart_int "$_sp_out" Offline_Uncorrectable Media_and_Data_Integrity_Errors)
    _sp_cr=$(_smart_int "$_sp_out" UDMA_CRC_Error_Count)
...
smart_choice() {
    # $1 = label (Disk N), $2 = context line, $3.. = disk/verdict/reason
    # (אופציונלי, למסך הגרפי של ה-cloner). מצייר תפריט ASCII (כמו
    # buildmenu.sh) ומחזיר את ההכרעה: replace / rescue / skip. הקלט
    # נקרא מ-stdin, ולכן הפונקציה נבדקת בהזרקת מקש. EOF/ריק = skip,
    # הברירה הזהירה (לא כותבים על דיסק חשוד בלי החלטה מפורשת).
    #
    # ‏#666+GUI: כשיש מסך cloner חי, ההכרעה נלקחת ממנו במקום מ-stdin —
    # מסלול קלט אחד, לא שניים. אם המסך נעלם, `gui_smart_choice` נכשל
    # והתפריט הטקסטואלי נכנס, עם אותה ברירת מחדל בטוחה (EOF = skip).
    if [ "$#" -ge 5 ] &&
    ...
    while :; do
        ...
        # הפלט ב-`$( )`, ותפריט על stdout היה נבלע לתוך ההכרעה. שני
        # ה-fd מגיעים לאותה קונסולה על ה-TTY של הסוכן.
        {
            ui_clear; ui_header
            echo "  $1: $2"
            echo
        ...
        read -r _sc_c || { echo skip; return; }
        case "$_sc_c" in
            1) echo replace; return ;;
            2) echo rescue;  return ;;
            3) echo skip;    return ;;
...
smart_send_event() {
    ...
    log "disk_event for $2 was not delivered -- continuing"
    return 0
}

smart_preflight() {
    # $1 = session id, $2.. = disks. השער לפני כתיבה: probe לכל דיסק,
    # disk_event לכל דיסק, ותפריט בחירה על fail/warn (בתור, אחד-אחד).
    ...
    _pf_write=""
    for _pf_d in "$@"; do
        # תיקיית היעד קיימת כבר מ-target_init שקדם לשער, אבל ה-mkdir כאן
        # הופך את השער לעצמאי -- target_set חייב תיקייה, וקפיאה על היעדרה
        # תהיה כשל שקט (עיקרון 5).
        ...
        case "$_pf_v" in
            ...
            warn|fail)
                _pf_dec=$(smart_choice "$(smart_disk_label "$_pf_d")" \
                    "SMART $_pf_v ($_pf_r)$([ "$_pf_r" = crc ] && echo ' -- check the cable')" \
                    "$_pf_d" "$_pf_v" "$_pf_r")
                case "$_pf_dec" in
                    rescue)
