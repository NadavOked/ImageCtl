...
int main(int argc, char **argv)
{
     ...
     * minutes. Junk or zero would evict every full drawer at once, and an
     * overflow (strtoll saturates to LLONG_MAX, ERANGE) would push the ceiling
     * to millions of years and silently disable eviction altogether -- both
     * are refused out loud and the default kept, never taken on (principle 5). */
    const char *stall_env = getenv("FANOUT_STALL_EVICT_MS");
    if (stall_env) {
        char *end;
        errno = 0;
        long long configured = strtoll(stall_env, &end, 10);
        if (end != stall_env && *end == '\0' && configured > 0 && errno != ERANGE)
            stall_evict_ms = configured;
        else
            fprintf(stderr, "fanout: ignoring invalid FANOUT_STALL_EVICT_MS\n");
    }
