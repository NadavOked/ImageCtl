...
#define KEY(k) (strcmp(key, k) == 0)
        if (KEY("disk") && s->ndisks < MAX_DISKS) {
            ...
            /* cdisk=port|dev|model|size|smart -- the cloner's idle inventory.
             * NOT "disk=" -- that key already holds the build machine's local
             * disk list (s->disks) and would swallow these first (#688). The
             * 5th field (#709) is optional so an older agent still parses. */
            int nf = split(val, f, 5);
            if (nf < 4) continue;
            ...
            id->size = size;
            /* Only warn/fail read as unhealthy; anything else -- passed, an
             * absent field, an unknown string -- is treated as unchecked. */
