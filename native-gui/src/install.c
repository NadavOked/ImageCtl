...
/* #1206: the keyboard ring. The ESXi web console delivers the arrows but
 * swallows Tab, so Up/Down (and Tab/Shift+Tab) walk every control on the
 * screen -- radio options, fields, the inline check button. Landing on a
 * radio option selects it: the highlighted option is the chosen one, as in
 * the disk list. The disk screen keeps its own handler below. */
#define INSTALL_MAX_STOPS (INSTALL_MAX_NICS+8)
static int stops(InstallState *s,int *ids){int n=0;if(s->view==INSTALL_ROLE){ids[n++]=HIT_INSTALL_PRIMARY;ids[n++]=HIT_INSTALL_SECONDARY;if(s->secondary){ids[n++]=HIT_INSTALL_PRIMARY_URL;ids[n++]=HIT_INSTALL_CHECK_PRIMARY;}return n;}if(s->view==INSTALL_NETWORK){for(int i=0;i<s->nnics;i++)ids[n++]=HIT_INSTALL_NIC_BASE+i;ids[n++]=HIT_INSTALL_DHCP;ids[n++]=HIT_INSTALL_STATIC;}return n+fields(s,ids+n);}
static int home(InstallState *s){int ids[8];if(fields(s,ids))return ids[0];if(s->view==INSTALL_ROLE)return s->secondary?HIT_INSTALL_SECONDARY:HIT_INSTALL_PRIMARY;if(s->view==INSTALL_NETWORK&&s->nic>=0)return HIT_INSTALL_NIC_BASE+s->nic;return 0;}
static void land(App *a,int id){InstallState *s=&a->install;a->focus=id;if(id==HIT_INSTALL_PRIMARY)s->secondary=0;else if(id==HIT_INSTALL_SECONDARY)s->secondary=1;else if(id==HIT_INSTALL_DHCP)s->static_mode=0;else if(id==HIT_INSTALL_STATIC){s->static_mode=1;seed_from_nic(s);}else if(id>=HIT_INSTALL_NIC_BASE&&id<HIT_INSTALL_NIC_BASE+s->nnics){s->nic=id-HIT_INSTALL_NIC_BASE;seed_from_nic(s);}}
static void move_focus(App *a,int dir){int ids[INSTALL_MAX_STOPS],n=stops(&a->install,ids),at=-1,h=home(&a->install);if(!n)return;for(int i=0;i<n;i++)if(ids[i]==a->focus)at=i;if(at<0){for(int i=0;i<n;i++)if(ids[i]==h)at=i;if(at>=0){a->focus=h;return;}at=dir>0?-1:0;}land(a,ids[(at+dir+n)%n]);}
...
static void refocus(App *a){a->focus=home(&a->install);}
...
if(s->view==INSTALL_DISK&&e->type==UIEV_KEY&&(e->key==KEYSYM_UP||e->key==KEYSYM_DOWN)){int d=s->disk,old=d,step=e->key==KEYSYM_DOWN?1:-1;for(int i=0;i<s->ndisks;i++){d+=step;if(d<0)d=s->ndisks-1;if(d>=s->ndisks)d=0;if(!s->disks[d].iso){s->disk=d;if(d!=old)s->disk_ack=0;break;}}return 0;}
/* #1206: Space outside a text field presses the focused inline button, and
 * on the network screen flips DHCP/static without walking past the other
 * NIC rows (walking would re-select them); static moves focus to the IP. */
if(e->type==UIEV_CHAR&&e->ch==' '){size_t n;if(!field(s,a->focus,&n)){if(s->view==INSTALL_ROLE&&s->secondary&&a->focus==HIT_INSTALL_CHECK_PRIMARY)check_primary_call(a,b);else if(s->view==INSTALL_NETWORK){if(!s->static_mode)choose(a,b,HIT_INSTALL_STATIC);else{s->static_mode=0;if(a->focus==HIT_INSTALL_STATIC)a->focus=HIT_INSTALL_DHCP;}}return 0;}}
if(e->type==UIEV_CHAR){size_t n;char *v=field(s,a->focus,&n);if(v&&strlen(v)+1<n){size_t z=strlen(v);v[z]=(char)e->ch;v[z+1]=0;}return 0;}if(e->type!=UIEV_KEY)return 0;
    /* scrolling owns the arrows only where the output box is on screen: a
     * technical_open left over from a failed run must not eat them later */
    int output=s->view==INSTALL_FAILED||(s->view==INSTALL_PROGRESS&&s->technical_open);
    if(e->key==KEYSYM_ESC){back(a);return 0;}if(e->key==KEYSYM_UP&&output){choose(a,b,HIT_INSTALL_OUTPUT_UP);return 0;}if(e->key==KEYSYM_DOWN&&output){choose(a,b,HIT_INSTALL_OUTPUT_DOWN);return 0;}if(e->key==KEYSYM_BACKSPACE){size_t n;char *v=field(s,a->focus,&n);if(v){size_t z=strlen(v);if(z)v[--z]=0;}return 0;}if(e->key==KEYSYM_ENTER){if(s->view!=INSTALL_PROGRESS)choose(a,b,s->view==INSTALL_FAILED?HIT_INSTALL_RETRY:HIT_INSTALL_NEXT);return 0;}if(e->key==KEYSYM_TAB||e->key==KEYSYM_BACKTAB||e->key==KEYSYM_UP||e->key==KEYSYM_DOWN)move_focus(a,e->key==KEYSYM_TAB||e->key==KEYSYM_DOWN?1:-1);return 0;}
