...
static int room_disk_healthy(const RoomDrawer *d) {
    return strcmp(d->smart, "warn") != 0 && strcmp(d->smart, "fail") != 0;
}
...
static void room_machine_draw(App *a, cairo_t *cr, const Machine *m, int mi,
    ...
    for (int port = 1; port <= slots; port++) {
        ...
        Rgb fill = t->led_idle;
        if (m->awake && d)
            fill = room_disk_healthy(d) ? t->led_ok : ROOM_BAD;
...
/* #709: a disk's health colour, same convention as the room grid -- only
 * warn/fail read as unhealthy (red); passed/unchecked/anything else is green. */
static Rgb idle_smart_color(const Theme *t, const char *smart) {
    return (!strcmp(smart, "warn") || !strcmp(smart, "fail")) ? ROOM_BAD : t->led_ok;
}
...
void screen_cloner(App *a, cairo_t *cr, double W, double H, double head_h) {
    ...
    if (panel) {
        char pt[96], ps[256];
        if (s->smart_port > 0) snprintf(pt, sizeof pt, "דיסק %d — בעיית SMART", s->smart_port);
        else                   snprintf(pt, sizeof pt, "דיסק — בעיית SMART");
        ...
        sp_btn[1] = btn_label(cr, "כתוב בכל זאת");
        sp_btn[2] = btn_label(cr, "דלג");
    ...
    if (s->ndrawers == 0 && !panel && s->nidisks > 0) {
        for (int k = 0; k < s->nidisks; k++) {
            ...
            if (id->port > 0)
                snprintf(ln, sizeof ln, "דיסק %d (SATA %d): %s · %s",
                         id->port, id->port - 1, id->model[0] ? id->model : "דיסק", sz);
            else
                snprintf(ln, sizeof ln, "%s · %s", id->model[0] ? id->model : "דיסק", sz);
    ...
    if (s->ndrawers == 0) {
        if (n_idle > 0) {
            for (int k = 0; k < n_idle; k++) {
                ...
                /* #709: green = healthy disk, red = SMART warn/fail */
                Rect led = { x + inner - 12, y + (idle_line[k].h - 10) / 2, 10, 10 };
                draw_fill_rrect(cr, led, 3, idle_smart_color(t, s->idisks[k].smart));
                y += idle_line[k].h + 10;
    ...
    if (panel) {
        ...
        /* buttons from the right: [Replace] [Rescue] [Skip] */
        static const int kinds[3] = { BTN_PRIMARY, BTN_PLAIN, BTN_PLAIN };
        static const int ids[3]   = { HIT_SMART_REPLACE, HIT_SMART_RESCUE, HIT_SMART_SKIP };
