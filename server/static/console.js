...
async function refreshStatus() {
  let data;
  try { data = await api("/overview"); } catch (e) { markStatusStale(); return; }
  if (!ME) return;
...
async function loadMachines() {
  try {
    [MACHINES, GROUPS] = await Promise.all([api("/machines"), api("/groups")]);
    populateSidebarGroups();
    updateAlertBadge();
...
async function loadHealth() {
  try {
    ...
    updateAlertBadge();
    if (current === "home" || current === "health" || current === "ports") renderCurrent();
  } catch (e) {
...
function populateSidebarImages() {
  ...
  tree.innerHTML = FOLDERS.map((f, i) => {
    ...
    if (!imgs.length) {
      return `<div class="inventory-node"><span class="tree-arrow-sp"></span><span>${uiIcon("image")}</span><span>${esc(f.name)}</span></div>`;
    }
    const rows = imgs.map((im) => {
      const idEnc = encodeId(im.id || "");
      return `<div class="inventory-node" onclick="openImageDetail('${idEnc}')"><span class="tree-arrow-sp"></span><span>${uiIcon("image")}</span><span>${esc(im.name)}</span></div>`;
    }).join("");
    return `<div class="inventory-node" onclick="event.stopPropagation();toggleInventoryGroup(this,'${fid}')"><span class="tree-arrow" data-open="${open}">${open ? "▾" : "▸"}</span><span>${uiIcon("image")}</span><span>${esc(f.name)}</span></div><div id="${fid}" class="inventory-children"${open ? "" : " hidden"}>${rows}</div>`;
  }).join("");
}
...
function fillSidebarTree(treeId, groups, machines, icon) {
  ...
  tree.innerHTML = groups.map((g) => {
    ...
    if (!kids.length) {
      return `<div class="inventory-node" onclick="selectMachinesGroup('${gidEnc}')"><span class="tree-arrow-sp"></span><span>${uiIcon(icon)}</span><span>${esc(g.label)}</span></div>`;
    }
    const childHtml = kids.map((m) => {
      const macEnc = encodeId(m.mac);
      const label = machineName(m) || m.mac;
      return `<div class="inventory-node" onclick="openMachineDetail('${macEnc}')"><span class="tree-arrow-sp"></span><span>${uiIcon("machine")}</span><span>${esc(label)}</span></div>`;
    }).join("");
    return `<div class="inventory-node"><span class="tree-arrow" data-open="${open}" onclick="event.stopPropagation();toggleInventoryGroup(this.closest('.inventory-node'),'${esc(boxId)}')">${open ? "▾" : "▸"}</span><span onclick="selectMachinesGroup('${gidEnc}')" style="display:contents"><span>${uiIcon(icon)}</span><span>${esc(g.label)}</span></span></div><div id="${esc(boxId)}" class="inventory-children"${open ? "" : " hidden"}>${childHtml}</div>`;
  }).join("");
}
...
function images() {
  ...
  if (!IMAGES.length) {
    ...
    const rows = list.map((r) => {
      ...
      const size = fmtBytes(r.total_compressed_bytes);
    ...
    tableBody = `<table class="table"><thead><tr><th>שם</th><th>מערכת</th><th>גודל</th><th>גרסה</th><th>SHA-256</th><th>עודכן</th><th>סטטוס</th><th>פעולות</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
...
function openImageDetail(id) {
  ...
  const size = fmtBytes(img.total_compressed_bytes);
...
function populateSidebarNics() {
  ...
  tree.innerHTML = nics.map((n) => {
    ...
    return `<div class="inventory-node${on}" onclick="selectSidebarNic('${esc(n.name)}')"><span class="tree-arrow-sp"></span><span>${uiIcon("network")}</span><span dir="ltr">${esc(n.name)}</span></div>`;
  }).join("");
}
...
function portListenView(def, check) {
  if (!check) return { cls: "warn", text: "לא אומת", detail: "אין בדיקת האזנה ב-/health" };
  if (check.state === "off" || check.state === "unknown") {
    return { cls: "warn", text: "לא אומת", detail: check.detail || "" };
  }
  if (def.healthId === "tftp_port") {
    if (check.state === "ok") return { cls: "ok", text: "פתוח", detail: check.detail || "" };
    if (check.state === "bad") return { cls: "err", text: "סגור", detail: check.detail || "" };
    return { cls: "warn", text: "פתוח", detail: check.detail || "" };
  }
  if (def.healthId === "server") {
    if (check.state === "ok") return { cls: "ok", text: "פתוח", detail: check.detail || "" };
    if (check.state === "bad") return { cls: "err", text: "לא עונה", detail: check.detail || "" };
    return { cls: "warn", text: "לא אומת", detail: check.detail || "" };
  }
  if (def.healthId === "udp_sender") {
    if (check.state === "ok") return { cls: "", text: "לא משדר", detail: check.detail || "" };
    if (check.state === "warn") return { cls: "warn", text: "פתוח", detail: check.detail || "" };
    if (check.state === "bad") return { cls: "err", text: "פתוח", detail: check.detail || "" };
    return { cls: "warn", text: "לא אומת", detail: check.detail || "" };
  }
  return { cls: "warn", text: "לא אומת", detail: check.detail || "" };
}
...
function ports() {
  if (!HEALTH) return pagePlaceholder();
  const defs = [
    { name: "HTTP", port: "8080", proto: "tcp", desc: "boot + API", healthId: "server" },
    { name: "HTTP", port: "8081", proto: "tcp", desc: "קונסולת ניהול", healthId: null },
    { name: "TFTP", port: "69", proto: "udp", desc: "bootloader", healthId: "tftp_port" },
    { name: "PXE", port: "4011", proto: "udp", desc: "PXE proxy", healthId: null },
    { name: "Multicast", port: "9000–9001", proto: "udp", desc: "שידור אימג׳", healthId: "udp_sender" },
  ];
  const soon = `title="בקרוב (נדרש endpoint)"`;
  const cards = defs.map((def) => {
    const check = def.healthId ? healthCheckById(def.healthId) : null;
    const view = portListenView(def, check);
    return `<div class="span-6"><div class="card">
      <div class="card-h"><span>${esc(def.name)} <small class="mono" dir="ltr">${esc(def.port)}/${esc(def.proto)}</small></span><span class="status ${view.cls}"><i></i>${esc(view.text)}</span></div>
      <div class="card-b">
        <p style="margin:0 0 10px;color:var(--muted);font-size:11px">${esc(def.desc)}${view.detail ? " — " + esc(view.detail) : ""}</p>
...
const pages = {
  ...
  monitor: { crumb: "מוניטור", title: "מוניטור", desc: "תצוגת תחנה לפי MAC", tabs: ["תצוגה"], render: pagePlaceholder },
  drivers: { crumb: "דרייברים", title: "דרייברים", desc: "ניהול דרייברים למחשבים — הזרקה לפי דגם (בסגנון FOG)", tabs: ["דרייברים"], render: pagePlaceholder },
  ...
  ports: { crumb: "פורטים", title: "פורטים", desc: "פורטי HTTP, TFTP, PXE ומולטיקאסט — מצב האזנה כפי שנקרא", tabs: ["סקירה"], render: ports, load: loadHealth },
...
function layout(page, index = 0) {
  ...
  return `<div class="breadcrumbs"><span>שרת אימג'ים</span><span class="sep">/</span><strong>${page.crumb}</strong></div><div class="toolbar"><div class="title-block"><h1>${page.title}</h1><p>${page.desc}</p></div><div class="toolbar-actions"><button class="btn" onclick="refreshPage()">${uiIcon("refresh")} רענון</button><div class="rel"><button class="btn primary" onclick="openAction()">+ פעולה</button><div id="pageActionMenu" class="action-menu"></div></div></div></div><div class="vcenter-tabs">${page.tabs.map((t, i) => `<div class="vcenter-tab ${i === index ? "active" : ""}" onclick="activateTab(${i})">${t}</div>`).join("")}</div><div class="scroll">${stale}${tabRender(current, index)}<div class="footer-note">ImageCtl Console</div></div>`;
}
...
function selectPageById(id) { selectPage(document.querySelector(`[data-page="${id}"]`), id); }
...
function openAction() {
  ...
  menu.innerHTML = (actions[current] || []).map(([label, fn]) =>
    `<button onclick="${fn};document.getElementById('pageActionMenu').style.display='none'">${label}</button>`).join("");
...
function openMachineDetail(mac) {
  ...
  const name = machineName(m) || m.mac;
  const klass = groupLabel(machineGroupId(m));
  openDrawer("תחנה — " + name, `<div class="detail-grid"><div class="detail-box"><span class="k">שם</span><span class="v">${esc(name)}</span></div><div class="detail-box"><span class="k">כיתה</span><span class="v">${esc(klass)}</span></div><div class="detail-box"><span class="k">MAC</span><span class="v">${esc(m.mac)}</span></div></div><div class="section-title">פעולות</div><div class="action-strip"><button class="btn" onclick="renameMachine('${macEnc}')">שינוי שם</button><button class="btn" disabled title="דורש endpoint — בקרוב">עריכת MAC</button><button class="btn" onclick="monitorMachine('${macEnc}')" ${["build","cloner"].includes((GROUPS || []).find(g=>g.id===machineGroupId(m))?.role) ? "" : "disabled"}>מוניטור</button><button class="btn" disabled title="בקרוב">Wake-on-LAN</button><button class="btn" disabled title="בקרוב">בדוק PXE</button><button class="btn danger" disabled title="בקרוב">אתחול</button></div>`);
}
...
async function createMachine() {
  ...
  try {
    await post("/machines", { mac, name, group_id });
    closeModal();
    toast("המחשב נוסף");
    await loadMachines();
  } catch (e) {
...
async function applyMachineImport() {
  ...
  try {
    ...
    closeModal();
    toast(`נשמרו ${r.saved}. נדחו ${(r.rejected || []).length}.`);
    await loadMachines();
  } catch (e) {
    toast(e.message);
  }
